...
NEXUS_SCRIPTS = {
    ...
    "upload": "./scripts/upload_raw.py"
}


def list_nx(repo_alias):
    os.chdir( NEXUS_APPDIR )

    list_cmd = "python3 {nexus_script} {repo_alias}".format( nexus_script=NEXUS_SCRIPTS["list"], repo_alias=repo_alias )
...
def delete_nx(repo_alias, filepath):
    os.chdir( NEXUS_APPDIR )

    delete_cmd = "python3 {download_script} {repo_alias} {filepath}".format( download_script=NEXUS_SCRIPTS["remove"], repo_alias=repo_alias, filepath=filepath ) 
...
def download_nx(repo_alias, filepath, local_filepath):
    os.chdir( NEXUS_APPDIR )

    download_cmd = "python3 {download_script} {repo_alias} {local_filepath} {filepath}" \
            .format( download_script=NEXUS_SCRIPTS["download"], repo_alias=repo_alias, local_filepath=local_filepath, filepath=filepath )
...
def upload_nx(repo_alias, local_filepath, filepath):
    os.chdir( NEXUS_APPDIR )

    upload_cmd = "python3 {upload_script} {repo_alias} {local_filepath} {filepath}" \
            .format( upload_script=NEXUS_SCRIPTS["upload"], repo_alias=repo_alias, local_filepath=local_filepath, filepath=filepath )
